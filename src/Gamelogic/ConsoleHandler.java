...
public class ConsoleHandler {
    ...
    private void getTeamMoves(Team team) {
    	...
    	for(Player p : team.getPlayers()) {
    		...
    		boolean valid = false; 
    		while (!valid) {
    			System.out.println("/nAktion für" + p.getName() + " (" + p.getRole() + ")");
    			...
    			switch (choice) {
    				case 1: //Laufen 
    					...
    					if (isValidMove(p, dx, dy)) {
    						p.getPosition().bewegung(dx, dy); 
    						System.out.println(p.getName() + "läuft nach (" + p.getPosition().getX() + ", " + p.getPosition().getY() + ")"); 
    						valid = true;					
    					} else {
    				...
    				case 2: //Passen
    					...
    					if(mate != null) {
    						System.out.println(p.getName() + "passt zu" + mate.getName());
    						ball.setOwner(mate); 
    						valid = true;
    					}
    					break;
    				...
    				case 3: //Torschuss
    					if (isNearGoal(p)) {
    						System.out.println(p.getName() + "schießt auf das Tor.");
    				...
    				case 5: // Energieaufladen
    					p.regenerateEnergy(); 
    					System.out.println(p.getName() + "lädt aktuell seine Energie auf.");
    					valid = true; 
    					break; 
    				default:
